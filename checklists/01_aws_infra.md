...
- [ ] ACM SSL 인증서 발급 [ap-northeast-2] (community.prideofmisfits.com) → ALB 연결
      ※ ALB 인증서는 ALB와 같은 리전이어야 함

## 1-3. 데이터베이스
- [ ] RDS PostgreSQL 인스턴스 생성 (프라이빗 서브넷, 자동 백업 7일, db.t3.small 이상 권장)
- [ ] RDS 생성 시 "Enable encryption" 체크 (AWS KMS 키) ※ 생성 후 변경 불가!
- [ ] RDS 파라미터 그룹에서 rds.force_ssl = 1 설정 (in-transit 암호화 강제)
- [ ] CloudWatch 알람: DatabaseConnections > 80% 임계값 설정
...
## 1-4. 스토리지 & CDN
- [ ] S3 버킷 생성 (mfp-assets) — "퍼블릭 액세스 전체 차단" 활성화
- [ ] CloudFront OAC(Origin Access Control) 생성 → S3 오리진에 연결
- [ ] S3 버킷 정책에 CloudFront OAC 전용 허용 추가 (Principal: cloudfront.amazonaws.com)
- [ ] S3 직접 URL(s3.amazonaws.com)로 접근 시 403 반환 확인
- [ ] ACM SSL 인증서 발급 [us-east-1] (community.prideofmisfits.com) → CloudFront 연결
      ※ CloudFront는 반드시 us-east-1 인증서만 허용 — 다른 리전 불가
